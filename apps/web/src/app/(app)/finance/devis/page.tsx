...
import { db } from "@/db";
import { devis as devisTable } from "@/db/schema";
...
export default async function DevisPage({
  ...
  const devis = await db
    .select()
    .from(devisTable)
    .where(eq(devisTable.orgId, orgId))
    .orderBy(desc(devisTable.dateCreation))
    .limit(200);
  return (
    <div className="space-y-6">
      <FinanceHeader
        title="Devis"
        subtitle="Suivez vos devis et relances"
        primaryAction={<Link href="/finance/devis/nouveau">Nouveau devis</Link>}
      />
      <Card>
                ...
                .map((row) => (
                  <TableRow key={row.numero}>
                    ...
                    <TableCell>
                      <Badge
                        variant={
                          row.statut === "envoye" ? "secondary" : "outline"
                        }
                      >
                        {row.statut}
                      </Badge>
