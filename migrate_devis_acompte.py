#!/usr/bin/env python3
"""
Migration : Ajouter les colonnes d'acompte à la table devis
"""
...
def migrate_devis_acompte():
    """Ajouter les colonnes d'acompte à la table devis"""
    ...
    try:
        ...
        logger.info(f"\n📊 Colonnes actuelles de la table devis : {len(columns)}")
        ...
        for col_name, col_type in new_columns:
            if col_name in columns:
                ...
                try:
                    sql = f"ALTER TABLE devis ADD COLUMN {col_name} {col_type}"
                    cursor.execute(sql)
...
if __name__ == '__main__':
    logger.info("\n" + "="*60)
    logger.info("  MIGRATION : Acomptes pour Devis")
    logger.info("="*60 + "\n")
    
    success = migrate_devis_acompte()
    
    if success:
        logger.info("\n✅ Vous pouvez maintenant redémarrer l'application")
        logger.info("   → kill <PID> && python3 app.py")
    else:
